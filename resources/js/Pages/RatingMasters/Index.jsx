...
const Index = ({ ratingMasters }) => {
    const { auth } = usePage().props;

    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">評価マスター一覧</h2>}
        >
            <Head title="評価マスター一覧" />
                    ...
                    <div className="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                        <table className="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {ratingMasters.map(rating => (
                                    <tr key={rating.id}>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{rating.id}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{rating.rating_name}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{rating.description}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <InertiaLink
                                                href={`/rating-masters/${rating.id}/edit`}
                                            ...
                                            <form action={`/rating-masters/${rating.id}`} method="POST" className="inline">
